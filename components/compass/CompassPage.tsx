...
export default function CompassPage() {
  ...
  const petalData = useMemo(() => {
    const maxCount = Math.max(
      ...VALUES.map((v) => choices.filter((c) => c.valueId === v.id).length),
      1
    )
    return VALUES.map((v) => {
      const count = choices.filter((c) => c.valueId === v.id).length
      return { value: v, count, ratio: count / maxCount }
    })
  }, [choices])

  const topValues = useMemo(
    () => [...petalData].filter((d) => d.count > 0).sort((a, b) => b.count - a.count).slice(0, 3),
    [petalData]
  )

  // SSR 阶段只渲染骨架，避免 hydration mismatch
  if (!mounted) {
    return (
    ...
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-[#FAFBFE] to-[#F0F2F8]">
      ...
      >
        {[
          ...
          <div key={s.label} className="flex flex-col items-center rounded-2xl bg-white py-3 shadow-sm">
      ...
      >
        {totalChoices === 0 ? (
          <div className="flex flex-col items-center py-14 text-center px-6">
            ...
            <p className="text-sm text-gray-400">每做一次选择，就会有一片花瓣出现</p>
          </div>
        ) : (
          <>
          ...
          <div className="space-y-2">
            {topValues.map((d, rank) => {
              ...
              return (
                ...
                >
                  <span className="text-xs font-bold text-gray-300 w-4 shrink-0">#{rank + 1}</span>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-gray-400 shrink-0 ml-2">{d.count}次 · {pct}%</span>
                    </div>
